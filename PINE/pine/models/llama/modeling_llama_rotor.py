...
class LlamaRotaryEmbedding(nn.Module):
    ...
    @torch.no_grad()
    def forward_prev(self, x, position_ids):
        if "dynamic" in self.rope_type:
            self._dynamic_frequency_update(position_ids, device=x.device)
...
class LlamaAttentionWithPSNew(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.is_causal = True
        self.sorting_method = self.config.sorting_method
    ...
    def forward(
        ...
        **kwargs,
        ...
        # all_position_ids: [prefix_len + doc_len_total + suffix_len]
        # self_region_mask: [doc_len_total, doc_len_total], 1 and 0 mask.

        doc_range = kwargs.get('doc_range')  # [num_docs, max_doc_len]
        doc_mask = kwargs.get('doc_mask')  # [num_docs, max_doc_len]
        doc_select = kwargs.get('doc_select')  # [doc_len_total]
        all_position_ids = kwargs.get('all_position_ids')  # [prefix_len + doc_len_total + suffix_len]
        self_region_mask = kwargs.get('self_region_mask')  # [doc_len_total, doc_len_total]
        global_ordering = kwargs.get('global_ordering')
        ...
        doc_range_region = (doc_range - doc_range[0, 0]) * doc_mask  # [num_docs, max_doc_len]
        ...
        if past_key_value.get_seq_length(self.layer_idx) == 0:
            ...
            if False:
                ...
                ### RoToR change  ###
                self.determ = global_ordering
                ndocs = len(global_ordering)
                det_ord = global_ordering + global_ordering
                template = []
                for i in range(ndocs):
                    cur_idx_index = det_ord.index(i)
                    template.append([det_ord[cur_idx_index:cur_idx_index+ndocs]])
                template = torch.tensor(template, device=query_states.device, dtype=torch.long).squeeze(dim=1)
                #RoToR_attn_weights_doc2doc_sort = template.repeat(1,32,1,1)
                RoToR_attn_weights_doc2doc_sort = template.repeat(1,1,1,1)
                # RoToR: change end
                attn_weights_doc2doc_sort = RoToR_attn_weights_doc2doc_sort
                attn_weights_doc2doc_sort_inverse = torch.argsort(attn_weights_doc2doc_sort, dim=-1)  # [bs, heads, num_docs, num_docs]
                ...
                position_shift = torch.gather(position_shift, dim=-1, index=attn_weights_doc2doc_sort_inverse)  # [bs, heads, num_docs, num_docs]
                # Now position_shift[:, :, i, j] means the position shift of doc j when computing attention between i and j
                #kwargs = {'all_position_ids', 'doc_range', 'doc_mask', 'doc_select', 'self_region_mask'}
            ...
            for doc_id in range(num_docs):
                ...
                doc_position = all_position_ids[:, None, :].expand(bs, 1, all_input_len).clone()  # [bs, heads, prefix_len + doc_len_total + suffix_len]
                doc_position[:, :, doc_st[0]: doc_ed[-1]] -= doc_position_shift  # [bs, heads, doc_len_total]
                doc_position = doc_position[:, :, :doc_ed[-1]]  # [bs, heads, prefix_len + doc_len_total]
                doc_cos, doc_sin = self.rotary_emb(value_states, doc_position.squeeze(0))  # [bs, heads, prefix_len + doc_len_total, hidden_dim]
                doc_cos = doc_cos.unsqueeze(1)
                doc_sin = doc_sin.unsqueeze(1)
                doc_q = query_states[:, :, doc_st[doc_id]: doc_ed[doc_id], :]  # [bs, heads, doc_len[doc_id], hidden_dim]
                doc_q = ((doc_q * doc_cos[:, :, doc_st[doc_id]: doc_ed[doc_id], :]) + (rotate_half(doc_q) * doc_sin[:, :, doc_st[doc_id]: doc_ed[doc_id], :]))  # [bs, heads, doc_len[doc_id], hidden_dim]
                doc_k = key_states[:, :, :doc_ed[-1], :]
                doc_k = repeat_kv(doc_k, self.num_key_value_groups)  # [bs, heads, prefix_len + doc_len_total, hidden_dim]  # TODO: head num consistency?
                doc_k = (doc_k * doc_cos) + (rotate_half(doc_k) * doc_sin)  # [bs, heads, prefix_len + doc_len_total, hidden_dim]
                doc_v = value_states[:, :, :doc_ed[-1], :]
                doc_v = repeat_kv(doc_v, self.num_key_value_groups)  # [bs, heads, prefix_len + doc_len_total, hidden_dim]

                doc_attn_weights = torch.matmul(doc_q, doc_k.transpose(2, 3)) / math.sqrt(self.head_dim)  # [bs, heads, doc_len[doc_id], prefix_len + doc_len_total]
                ...
                doc_attn_outputs = torch.matmul(doc_attn_weights, doc_v) # [bs, heads, doc_len[doc_id], hidden dim]
        ...
        suffix_query_states = query_states[:, :, doc_ed[-1]:, :] if is_forward_pass else query_states  # [bs, heads, suffix_q_len, hidden_dim]
        suffix_q_len = suffix_query_states.shape[-2]

        ### RoToR: change new  ###
        #print(f"doc_range: {kwargs['doc_range']}")
        #print(f"input_ids: {input_ids[0].shape} {input_ids[0]}")
        global_ordering = torch.tensor(self.determ, device=input_ids.device)
        # RoToR: change end
        #attn_weights_per_doc_sort = deterministic_ordering.repeat(1,32,suffix_q_len,1)
        attn_weights_per_doc_sort = global_ordering.repeat(1,1,suffix_q_len,1)
        #if input_ids[0].shape[0] == 1:

        # attn_weights_per_doc_sort = RoToR_attn_weights_doc2doc_sort
        # RoToR change end
        ...
        position_shift = position_shift[:, :, :, doc_select]  # [bs, heads, suffix_q_len, doc_len_total]
        dynamic_position = all_position_ids[:, None, None, :].expand(bs, 1, suffix_q_len, all_input_len).clone() # [bs, 1, 1, prefix_len + doc_len_total + suffix_len]
        dynamic_position[:, :, :, doc_st[0]: doc_ed[-1]] -= position_shift
        ### CHANGE: RoToR ###
        #for i in range(dynamic_position.shape[-2]):
        #    dynamic_position[:,:,i,:] = dynamic_position[:,:,suffix_q_len-1,:].clone()
        dynamic_cos, dynamic_sin = self.rotary_emb(value_states, dynamic_position.squeeze(0).squeeze(0))  # [bs, heads, suffix_q_len, all_input_length, hidden_dim]
        dynamic_cos = dynamic_cos.unsqueeze(0).unsqueeze(0)
        dynamic_sin = dynamic_sin.unsqueeze(0).unsqueeze(0)
        dynamic_q = ((suffix_query_states * dynamic_cos[:, :, 0, -suffix_q_len:, :]) + (rotate_half(suffix_query_states) * dynamic_sin[:, :, 0, -suffix_q_len:, :]))  # [bs, heads, suffix_q_len, hidden_dim]
...
class LlamaPreTrainedModelWithPSNew(PreTrainedModel):
...
class LlamaModelWithPSNew(LlamaPreTrainedModelWithPSNew):
    """
    ...
    def forward(
        ...
        **ps_kwargs
        ...
        causal_mask = self._update_causal_mask(attention_mask, inputs_embeds, cache_position, past_seen_tokens, ps_kwargs)
    ...
    def _update_causal_mask(
        ...
        #    cache_position = torch.tensor([cache_position.shape[0]], device=input_tensor.device)
        #    cache_position = torch.arange(attention_mask.shape[1], device=input_tensor.device)
        ...
        #causal_mask[:, :] = 0.0
        # not generation
        #to_add = causal_mask.clone().transpose(dim0=2, dim1=3)
        #import pdb.set_trace()
        #torch.set_printoptions(linewidth=5000, threshold=100000)
        if False:
            if cache_position.shape[0] != 1:
                causal_mask_base = causal_mask.clone()
                doc_start = ps_kwargs['doc_range'][0][0]
                doc_end = ps_kwargs['doc_range'][-1].max()
                doc_len = doc_end-doc_start+1
                #to_add = causal_mask.clone().transpose(dim0=2, dim1=3)[:,:,:doc_len, :doc_len]
                to_add = causal_mask.clone()[:,:,:doc_len, :doc_len]
                torch.set_printoptions(linewidth=5000, threshold=100000)
                causal_mask[:, :, doc_start:doc_end+1, doc_start:doc_end+1] -= to_add
                doc_start_idxs = ps_kwargs['doc_range'][:, 0]
                causal_mask[:,:,doc_start:doc_end+1, doc_start:doc_end+1] = -causal_mask[:,:,doc_start:doc_end+1, doc_start:doc_end+1]
                for idx in doc_start_idxs:
                    causal_mask[:,:,idx,idx] += min_dtype
                causal_mask[:,:,:,doc_start:doc_end+1] = 0
                causal_mask[:,:,:,doc_start:doc_end+1] = -causal_mask[:,:,:,doc_start:doc_end+1]
                #causal_mask_base -= causal_mask_base
                #causal_mask_base = -causal_mask_base
...
class LlamaForCausalLMWithPSRoToR(LlamaPreTrainedModelWithPSNew):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaModelWithPSNew(config)
        self.vocab_size = config.vocab_size

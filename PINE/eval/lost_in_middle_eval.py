...
def main():
    ...
    parser.add_argument('--mode', type=str, choices=['ps','ours', 'origin'])
    parser.add_argument('--eval_data', type=str)
    parser.add_argument('--filename', type=str)
    parser.add_argument('--model_name', type=str)
    parser.add_argument('--start_idx', type=int, default=0)
    parser.add_argument('--prev_score', type=float, default=0)
    ...
    sys.path.append('/workspace/jenny/TableInvariance/PINE')
    ...
    if args.mode == 'origin':
        prompts = [template_adjust(d[0], model_name) for d in data[args.start_idx:args.eval_num]]
        outputs = model.generate(prompts, sampling_params)
        answers = [o.outputs[0].text for o in outputs]
        writer = open(dump_path, 'a+')
        score = args.prev_score*args.start_idx
        idx = 0
        for answer, gt in zip(answers, [d[2] for d in data[args.start_idx:args.eval_num]]):
            answer = answer.strip()
        ...
        print('Score: ', score/(args.start_idx + len(answers)))
    else:
        tot = args.start_idx
        score = args.prev_score*args.start_idx
        print(f"prev score: {score}")
        writer = open(dump_path, 'a+')
        for d in tqdm(data[args.start_idx:args.eval_num]):
            tot+=1
            origin_prompt = d[1]

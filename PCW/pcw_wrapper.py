...
try:
    ...
    from pcw_utils import n_tokens_in_prompt
except:
    from PCW.logits_processor import RestrictiveTokensLogitsProcessor
    from PCW.pcw_utils import n_tokens_in_prompt
...
class PCWModelWrapper:
    def __init__(self,
        ...
        self.model = model
        #self.debug_model = AutoModelForCausalLM.from_pretrained('meta-llama/Meta-Llama-3-8B-Instruct').to(device)
    ...
    def generate_original(self, contexts=None, task_text=None, contexts_cache=None,
            restrictive_logit_preprocessor=None, **kwargs):
        with torch.no_grad():
            input_ids= self.tokenizer(contexts[0]+task_text, add_special_tokens=False, return_tensors='pt').to(self.device)
            outputs = self.debug_model.generate(input_ids=input_ids['input_ids'],
                                      pad_token_id=self.tokenizer.eos_token_id,
                                      eos_token_id=[self.tokenizer.eos_token_id, self.tokenizer.convert_tokens_to_ids("<|eot_id|>")],
                                      **kwargs)[0]
            decoded_output = self.tokenizer.decode(outputs[input_ids['input_ids'].shape[1]:])

...
class LlamaForCausalLMPCW(LlamaForCausalLM, ABC):
    ...
    def prepare_inputs_for_generation(self,
        ...
        if attention_mask is not None and position_ids is None:
            ...
            cache_position = position_ids[0] # replace cache position for task tokens!
...
class LlamaModelPCW(LlamaModel, ABC):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayerPCW(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        attention_len = attention_mask.shape[1]
        position_len = cache_position.shape[0]
        if attention_len != position_len: # last task token
            position_ids_for_causal_mask = torch.arange(attention_len - position_len, attention_len, device=cache_position.device)
            causal_mask = self._update_causal_mask(attention_mask, inputs_embeds, position_ids_for_causal_mask, past_key_values, output_attentions)
        else:
...
class LlamaSdpaAttentionPCW(LlamaAttention):
    """
